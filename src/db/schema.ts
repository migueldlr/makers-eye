import { pgTable, pgPolicy, bigint, timestamp, jsonb, text, date, foreignKey, numeric, pgView } from "drizzle-orm/pg-core"
import { sql } from "drizzle-orm"



export const decklists = pgTable("decklists", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "decklists_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
...
export const tournaments = pgTable("tournaments", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "tournaments_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
...
export const cards = pgTable("cards", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "cards_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	name: text().notNull(),
	type: text().notNull(),
}, (table) => [
...
export const standings = pgTable("standings", {
	...
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	corpWins: bigint("corp_wins", { mode: "number" }).default(sql`'0'`).notNull(),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	corpLosses: bigint("corp_losses", { mode: "number" }).default(sql`'0'`).notNull(),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	corpDraws: bigint("corp_draws", { mode: "number" }).default(sql`'0'`).notNull(),
	corpIdentity: text("corp_identity").default(').notNull(),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	runnerWins: bigint("runner_wins", { mode: "number" }).default(sql`'0'`).notNull(),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	runnerLosses: bigint("runner_losses", { mode: "number" }).default(sql`'0'`).notNull(),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	runnerDraws: bigint("runner_draws", { mode: "number" }).default(sql`'0'`).notNull(),
	runnerIdentity: text("runner_identity").default(').notNull(),
	sos: numeric().notNull(),
	eSos: numeric("e_sos").notNull(),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	swissRank: bigint("swiss_rank", { mode: "number" }).default(sql`'0'`).notNull(),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
...
export const matches = pgTable("matches", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "matches_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	tournamentId: bigint("tournament_id", { mode: "number" }).notNull(),
...
export const identityNames = pgTable("identity_names", {
...
export const standingsMapped = pgView("standings_mapped", {	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
